...
export default function MyCareer(props: MyCareerInterface){
    const [journeyText, SetJourneyText] = useState("Scroll to reveal my journey...");
    ...
    useEffect(() => {
        if(props.value < 5 && props.value > 0){
            ...
            SetJourneyText("I graduated from The University of Reading in Biomedical Engineering, attaining a First")
            if(stateOfCareer != 1){
                SetStateOfCareer(1);
                ApplyConfettyFcn();
            }
        }else if(props.value >= 20 && props.value < 40){
            SetJourneyText("I then worked as a researcher at the University of Reading for 6 months")
            if(stateOfCareer != 2){
                SetStateOfCareer(2);
            }
        }else if(props.value >= 40 && props.value < 60){
            SetJourneyText("I got a scholarship to Imperial to study MSc Human and Biological Robotics (Merit)")
            if(stateOfCareer != 3){
                SetStateOfCareer(3);
                ApplyConfettyFcn();
            }
    ...
    return(
        <div className="border rounded-md border-[#00FFFF] bg-[#0C0A09] p-2 w-100/100 h-100/100" >
                <div className="flex flex-col justify-between m-2">
                    ...
                    <LinearProgress className = {`mt-10`} variant="determinate" value={props.value} />
                </div> 
        </div>
    )
}
